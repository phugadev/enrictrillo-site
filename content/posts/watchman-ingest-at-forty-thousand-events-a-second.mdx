...
The pipeline that came out of it, end to end:

<Diagram
  nodes={[
    { label: "Probes", wavelength: "systems", note: "HTTP, queue, disk" },
    { label: "Batcher", wavelength: "compute", note: "in-memory buffer" },
    { label: "Postgres", wavelength: "intelligence", note: "COPY writer" },
  ]}
  edges={[undefined, "flush: 2k rows or 250ms"]}
/>

<Callout variant="warning">
  Batching trades per-event latency for throughput. It's the right call for a health signal where the aggregate matters, not the individual event — don't reach for it on a path that needs a same-request answer.
</Callout>
...
```ts title="lib/ingest/batcher.ts" {3,12-13}
